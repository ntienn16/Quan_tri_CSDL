...
as
begin
	declare @MatKhauHash varchar(20);
	set @MatKhauHash=convert(varchar(50),HASHBYTES('md5',@MatKhau))
    if exists (select * from TaiKhoan where TenDN = @TenDN and MatKhau = @MatKhauHash and Quyen = 0)
        select 0 as code
    else if exists (select * from TaiKhoan where TenDN = @TenDN and MatKhau = @MatKhauHash and Quyen = 1)
        select 1 as code
    else if exists(select * from TaiKhoan where TenDN = @TenDN and MatKhau != @MatKhauHash) 
        select 2 as code
    else select 3 as code
end
exec SP_AuthoLogin admin,'!#/)zW¥§C‰JJ€Ã'
select * from TaiKhoan